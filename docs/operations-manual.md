...
| Self-hosted deploy runner | Settings → Actions → Runners | At least one online runner with `az`, `curl`, and `jq` — `_deploy.yml` targets `runs-on: [self-hosted]`, so deploys queue indefinitely without one. Dedicate it to this repo; prefer ephemeral runners (see [getting-started](getting-started.md#self-hosted-deploy-runner)) |
