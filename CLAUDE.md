...
Pipeline behavior also depends on repo settings outside these files: rulesets, workflow permissions (default token read-only; "Actions can create PRs" enabled for the back-merge step), the `AZURE_TENANT_ID`/`AZURE_SUBSCRIPTION_ID` repo variables, the per-environment `AZURE_CLIENT_ID`/`WEBAPP_NAME`/`RESOURCE_GROUP` variables (optional per env: `GATEWAY_URL` for gateway-fronted smoke tests, `DEPLOY_ALERT_WEBHOOK` for failure notifications), a registered **self-hosted deploy runner** (deploys queue forever without one), and required reviewers + `main`-only branch policy on the prod environment — see the "Repository settings" section of `docs/operations-manual.md`. Gotcha: jobs downstream of the promotion path's skipped `build` job need explicit `!failure() && !cancelled()` conditions (implicit `success()` fails on skipped ancestors).
