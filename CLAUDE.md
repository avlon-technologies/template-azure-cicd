...
- `_hotfix-support.yml` — shared `prepare` → `build` → `deploy` pipeline for the two near-identical hotfix and support flows. The thin `on-hotfix.yml` / `on-support.yml` entry workflows call it with a `branch-type` (`hotfix` | `support`) plus the trigger context (`event-name`, `ref-name`, `sha`, `run-number`), so one definition serves both: it auto-labels pushes (`X.Y.Z-hotfix.<run>` / `X.Y.Z-patch.<run>`), validates dispatched labels against the branch, reuses an existing `webapp-<sha>` artifact on redispatch, and deploys to stg only on `workflow_dispatch`.
